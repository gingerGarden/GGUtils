...
import importlib, os, sys, types, hashlib, re, warnings, logging, time
import github
...
def get_module_from_py(py_path:str, unique_module:bool=False)->types.ModuleType:
    ...
    """
    # py_path 무결성 확인
    if not os.path.exists(py_path) or not py_path.endswith(".py") or not os.path.isfile(py_path):
...
def _get_new_module(py_path:str, module_name:str)->types.ModuleType:
    ...
    try:
        ...
        raise ImportError(f"Failed to load or execute the module {module_name} from {py_path}: {e}") from e
    return module


def explore_modules_recursiverly(py_path:str, module_paths:Optional[List[str]]=None, visited:Optional[set]=None)->List[str]:
    """
    ...
    for module in module_list:
        ...
        # 재귀적으로 하위 모듈 탐색
        explore_modules_recursiverly(py_path=module_path, module_paths=module_paths, visited=visited)
    return module_paths
...
def get_module_list_in_py_file(py_path:str, public_package:bool=True)->List[str]:
    ...
    if public_package:
        return [module for module in modules if re.match(pattern=r"^(\.)+", string=module)]
    else:
        return list(modules)
...
class GithubHelper:
    ...
    @classmethod
    def download_repo_directory(cls, url:str, parents_path:str, token:Optional[str]=None):
        """
    ...
    def download_contents(
            cls, repo_ins:github.Repository.Repository, tree:str, parents_path:str, 
            wait:Optional[int]=None, retry:int=0, max_retry:int=5, encoding:str='utf-8'
        ):
        """
        ...
        try:
            ...
            if retry < max_retry:
                logging.warning(f"Rate limit exceeded. Waiting... Retry {retry+1}/{max_retry}")
                time.sleep(wait)
                return cls.download_contents(repo_ins, tree, parents_path, wait=wait, retry=retry+1, max_retry=max_retry)
            else:
        ...
        for content in contents:
            ...
            if content.type == "file":
                ...
                cls.download_contents(repo_ins, content.path, parents_path, wait=wait, retry=0, max_retry=max_retry)
    

    @classmethod
    def download_file(cls, content:github.ContentFile.ContentFile, file_path:str, encoding:str='utf-8'):
        """
    ...
    def get_directory_tree(cls, url:str)->str:
        ...
        if match:
            ...
            raise ValueError(f"입력된 url에 github url의 tree 패턴인 {cls.tree_pattern}이 존재 하지 않습니다!")