...
import os, pickle, shutil, json, requests

import pandas as pd
import numpy as np


def do_or_load(savepath:str, fn:Callable, makes_new:bool=False, *args, **kwargs)->Any:
...
def file_download(url:str, local_path:str, kb:int=8, wait:int=10, overwrite:bool=False):
    """
    ...
    if os.path.exists(local_path):
        if not overwrite:
            print(f"'{local_path}'가 이미 존재합니다. 덮어쓰기를 원한다면 overwrite를 True로 입력하십시오.")
            return
    ...
    try:
        ...
        raise ConnectionError(f"File 다운로드 실패! {url}에서 타임아웃이 발생했습니다! {e}") from e
    except requests.exceptions.ConnectionError as e:
        raise ConnectionError(f"파일 다운로드 실패! {url}에 연결할 수 없습니다! {e}") from e
    except requests.exceptions.RequestException as e:
        raise ConnectionError(f"File 다운로드 실패! {url}이 다운로드 되지 않았습니다! {e}") from e
    except OSError as e:
        raise ConnectionError(f"파일 저장 중 오류가 발생했습니다: {local_path} - {e}") from e


class GetAbsolutePath:

    def __init__(self, extensions:Union[List[str], None]=None):
        """
    ...
    def get_all_path(self, parents_path:str)->List[str]:
        ...
        for root, _, files in os.walk(top=parents_path):
            sub_path_list = list_flatten([os.path.join(os.path.abspath(root), file) for file in files])
            stack.extend(sub_path_list)
        return stack


    def _extensions_handler(self, extensions:Union[List[str], None])->Union[List[str], None]:
        """
        입력된 extensions이 None이 아니라면 item을 소문자로 바꾼다.

        Args:
            extensions (Union[List[str], None]): 확장자들이 들어가 있는 list

        Returns:
            Union[List[str], None]: 소문자로 변환된 item들이 들어가 있는 list 또는 None
        """
        return [i.lower() for i in extensions] if type(extensions) == list else None
