...
def get_img_axis_size(img:np.ndarray)->Tuple[int, int]:
    ...
    if len(shape_tuple) == 2:
        ...
        raise ValueError("해당 함수는 2D 이미지(흑백, RGB, RGBA 등)를 대상으로 하고 있습니다. 입력된 배열은 해당 범위를 벗어납니다.")
    return x, y
...
def check_rgb_or_rgba(img:np.ndarray):
    ...
    if not (mask1 and mask2):
        raise ValueError("해당 함수의 입력 이미지는 BGR, RGB, RGBA를 대상으로 합니다. 입력 이미지의 shape을 확인해주십시오.")
...
def size_tuple_type_error(size_tuple:Optional[Tuple[int, int]]):
    ...
    if size_tuple is not None:
        ...
        mask3 = isinstance(size_tuple[0], int) and isinstance(size_tuple[1], int) if mask2 else False
        if not (mask1 & mask2 & mask3):
            raise TypeError("size_tuple은 (int, int) 형식의 튜플 또는 None이 입력되어야 합니다.")
...
def img_corners(
    ...
    if (img is None) and (x is None or y is None):
        raise ValueError("img, x, y의 모든 값들이 None으로 입력되지 않았습니다! img 또는 x, y의 값을 입력하십시오!")
...
class RGBTuple:
    ...
    def check(cls, target:Any)->bool:
        ...
        """
        return (
            isinstance(target, tuple) and 
            len(target) == 3 and 
            all(isinstance(i, int) for i in target) and 
            all(0 <= i <= 255 for i in target)
        )
...
class GetInDirectory:
    ...
    def __call__(self, index:int)->np.ndarray:
        ...
        if index > img_size-1 or index < 0:
            raise ValueError(f"입력된 index {index}는 총 이미지의 개수인 {img_size}의 범위를 넘어섭니다(index는 0부터이므로 {img_size-1}까지 입력). index를 수정하여 입력하십시오.")
