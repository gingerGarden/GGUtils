...
def show_img(
        img:np.ndarray, 
        title:str=None, title_color:str='limegreen', titlesize:int=15,
    ...
    if not isinstance(img, np.ndarray):
        raise TypeError("입력 데이터는 np.ndarray 타입이어야 하며, 이미지를 기대합니다.")
...
def show_imgs(img_list:List[np.ndarray], col:int=4, img_size:Tuple[int, int]=(3,3), show_number:bool=False):
    """
...
class Slider:
    def __init__(
            self, 
            imgs:Union[List[np.ndarray], np.ndarray], titles:Union[List[str], None]=None, 
            title_color:str='limegreen', title_size:int=10,
            figsize:Tuple[int, int]=(8, 6)
            ):
        """
        imgs를 Slider로 출력한다.
    ...
    def _show_img_use_index(self, idx):
        ...
        show_img(
            img, title=title, title_color=self.title_color, 
            fontsize=self.title_size, figsize=self.figsize
        )
    ...
    def _layout(self, value:int=0):
        ...
        slider = widgets.IntSlider(
            value=value, min=0, max=len(self.imgs)-1, 
